...
@protocol OCCastChannelHandler;

/**
 * An invalid request ID; if a method returns this request ID, it means that the request could
 * not be made.
 */
extern const NSInteger kOCInvalidRequestID;

/**
 * A OCCastChannel is used to send and receive messages that are tagged with a specific
 * namespace. In this way, multiple channels may be multiplexed over a single connection
 * to the device.
 * <p>
 * Subclasses should implement the @link OCCastChannel#didReceiveTextMessage: @endlink and/or
 * @link OCCastChannel#didReceiveBinaryMessage: @endlink methods to process incoming messages,
 * and will typically provide additional methods for sending messages that are specific to a
 * given namespace.
 *
 * @ingroup Messages
 */
@interface OCCastChannel : NSObject
...
/**
 * Designated initializer. Constructs a new OCCastChannel with the given namespace.
 *
