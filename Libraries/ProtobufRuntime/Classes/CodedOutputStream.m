...
- (instancetype)initWithOutputStream:(NSOutputStream*)output data:(NSMutableData*)data {
    ...
    _output = output;
    _buffer = [[RingBuffer alloc] initWithData:data];
	return self;
}

+ (instancetype)streamWithOutputStream:(NSOutputStream*)output bufferSize:(NSUInteger)bufferSize {
...
- (void)flush {
    NSAssert((_output != nil), @"Only call flush with an output stream.");
    
	if (_output != nil) {
		// We're writing to a single buffer.
		[_buffer flushToOutputStream:_output];
	}
}

- (void)writeRawByte:(uint8_t)value {
	while (![_buffer appendByte:value]) {
        [self flush];
	}
}

- (void)writeRawData:(const NSData*)data {
    int32_t length = 0; length += data.length;
	[self writeRawData:data offset:0 length:length];
}

- (void)writeRawData:(const NSData*)value offset:(int32_t)offset length:(int32_t)length {
	while (length > 0) {
		NSInteger written = [_buffer appendData:value offset:offset length:length];
		offset += written;
		length -= written;
		if (!written || length > 0) {
            [self flush];
...
- (void)writeDataNoTag:(const NSData*)value {
    int32_t length = 0; length += value.length;
	[self writeRawVarint32:length];
